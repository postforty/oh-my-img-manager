...
- **잘라내기(크롭/트리밍) 전용 섹션**: 피사체 주변의 불필요한 투명 여백을 원클릭으로 정밀 감지하는 **자동 트리밍**과 함께, **너비(W)/높이(H) 픽셀 직접 입력**, **가로세로 비율 유지 및 종횡비 프리셋(자유, 1:1, 4:3, 16:9)**, **8방향 리사이즈 핸들 마우스 드래그 크기 조절**, **크롭 박스 자유 이동**을 완벽하게 지원합니다.
...
- **다중 포맷 저장(PNG, WEBP, JPEG, GIF) & 자체 GIF 인코더 탑재**: 무손실 알파 투명도(PNG/WebP), 고압축 JPEG(투명도 미지원 시 흰색 밑바탕 자동 합성), 그리고 자체 경량 GIF89a 인코더를 통한 256색 및 1비트 투명도 GIF 저장을 모두 지원하며, 직관적인 실시간 품질/투명도 피드백 배지를 제공합니다.
...
5. 원하는 포맷(PNG, WEBP, JPEG, GIF)을 선택하고 [이미지 다운로드] 또는 [클립보드로 복사 (Ctrl+C)]를 눌러 결과물을 활용합니다.
...
### 🚀 v1.4.0 (`2026-09-26`)
**누끼 스튜디오 다중 저장 포맷(PNG, WEBP, JPEG, GIF) 지원 및 자체 GIF 인코더 탑재**
- **4대 주요 저장 포맷 선택 기능 추가**:
  - `PNG`: 무손실 알파 투명도 보존 (기본값)
  - `WEBP`: 최신 웹 표준 고효율 압축 및 알파 투명도 지원
  - `JPEG`: 불투명 고압축 포맷 지원 (투명 캔버스 내보내기 시 검은색 배경 결함을 방지하는 자동 흰색(#FFFFFF) 밑바탕 합성 알고리즘 탑재)
  - `GIF`: 자체 경량 GIF89a 인코더를 통한 256색 및 1비트 투명도 지원
- **경량 무의존성 GIF89a 인코더 엔진 탑재 (`lib/gif-encoder.js`)**:
  - 외부 라이브러리 없이 순수 JavaScript 기반 Median Cut 색상 양자화, 15비트 고속 LUT 매핑 및 표준 LZW 압축 엔진 구현
- **저장 포맷별 실시간 품질/투명도 피드백 배지**:
  - JPEG 선택 시 투명도 미지원 및 화이트 배경 대체 안내
  - GIF 선택 시 256색 및 1비트 투명도 계단 현상 안내 제공
- **버튼 및 UI 정돈**:
  - 단일 포맷 종속적인 `PNG 다운로드` 명칭을 직관적인 `이미지 다운로드`로 개선 및 드롭다운 확장자 중복 표기 제거

---

### 🚀 v1.3.0 (`2026-09-22`)
**이미지 잘라내기 수치 직접 설정 및 8방향 리사이즈 핸들 기능 추가**
- **잘라내기 패널 수치 설정 및 종횡비 프리셋**:
  - 가로(W)/세로(H) 픽셀 직접 입력 및 종횡비 프리셋(`자유`, `1:1`, `4:3`, `16:9`) 지원
  - 비율 유지 잠금(🔒) 활성화 시 왜곡 없는 엄격한 비율 보정 적용
- **8방향 리사이즈 핸들 및 자유 영역 이동**:
  - 크롭 선택 박스에 8개 방향 마우스 드래그 리사이즈 핸들 및 내부 영역 드래그 이동 기능 구현
- **양방향 동기화**:
  - 수치 입력 필드와 캔버스 드래그 영역 간 실시간 양방향 동기화 처리
